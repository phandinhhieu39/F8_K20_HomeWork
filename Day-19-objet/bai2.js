...
function getCustomerStatistics(customers, products, orders) {
  ...
  const customerStats = customers.map((customer) => {
    ...
    customerOrders.forEach((order) => {
      order.items.forEach((item) => {
        ...
        productSummary[item.productId].totalSpent += product.price * item.quantity;
      });
    });

    // totalSpent giảm dần
    const productsArr = Object.values(productSummary).sort(
      (a, b) => b.totalSpent - a.totalSpent
    );

    // Tính tổng tiền
    const totalSpent = productsArr.reduce((sum, p) => sum + p.totalSpent, 0);
  ...
  // Sắp xếp customer theo totalSpent giảm dần
